...
void Graph::loadGraph(string path) {
  ...
  for(i = 1; i <= numNodes; i++){
    vector<IP> adjacencies = {};
    pair<IP, vector<IP>> node = {ipList[i-1], adjacencies};
    adjList[i] = node;
  }
...
void Graph::addEdge(int v, int e){
  ...
  adjList[v].second.push_back(adjList[e].first);
  // Incrementa el grado de la IP
  adjList[v].first.increment();
  adjList[e].first.attack();
}
